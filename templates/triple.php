<?php
//
// Description
// -----------
// This function will output a pdf document as a series of thumbnails.
//
// Arguments
// ---------
//
// Returns
// -------
//
function qruqsp_tutorials_templates_triple($ciniki, $tnid, $categories, $args) {

    require_once($ciniki['config']['ciniki.core']['lib_dir'] . '/tcpdf/tcpdf.php');
    ciniki_core_loadMethod($ciniki, 'ciniki', 'images', 'private', 'loadImage');
    ciniki_core_loadMethod($ciniki, 'ciniki', 'images', 'private', 'loadCacheOriginal');
    ciniki_core_loadMethod($ciniki, 'ciniki', 'tenants', 'private', 'tenantDetails');

    //
    // Load tenant details
    //
    $rc = ciniki_tenants_tenantDetails($ciniki, $tnid);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    if( isset($rc['details']) && is_array($rc['details']) ) {   
        $tenant_details = $rc['details'];
    } else {
        $tenant_details = array();
    }

    //
    // Load INTL settings
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'tenants', 'private', 'intlSettings');
    $rc = ciniki_tenants_intlSettings($ciniki, $tnid);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $intl_currency_fmt = numfmt_create($rc['settings']['intl-default-locale'], NumberFormatter::CURRENCY);
    $intl_currency = $rc['settings']['intl-default-currency'];

    //
    // Create a custom class for this document
    //
    class MYPDF extends TCPDF {
        public $tenant_name = '';
        public $title = '';
        public $pagenumbers = 'no';
        public $coverpage = 'no';
        public $toc = 'no';
        public $toc_categories = 'no';
        public $doublesided = 'no';
        public $footer_height = 0;
        public $header_height = 0;
        public $footer_text = '';
        public $left_col_width = 80;
        public $right_col_width = 80;

        public function Header() {
            $this->SetFont('helvetica', 'B', 20);
            $this->SetLineWidth(0.25);
            $this->SetDrawColor(125);
            $this->setCellPaddings(5,1,5,2);
            if( $this->title != '' ) {
                $this->Cell(0, 18, $this->title, 'B', false, 'C', 0, '', 0, false, 'M', 'M');
            }
            $this->setCellPaddings(0,0,0,0);
        }

        // Page footer
        public function Footer() {
            // Position at 15 mm from bottom
            // Set font
            if( $this->pagenumbers == 'yes' ) {
                $this->SetY(-15);
                $this->SetFont('helvetica', 'I', 8);
                if( $this->toc == 'yes' ) {
                    $this->Cell(0, 8, $this->footer_text . '  --  Page ' . $this->getAliasNumPage().' of '.$this->getAliasNbPages(), 
                        0, false, 'C', 0, '', 0, false, 'T', 'B');
                } else {
                    $this->Cell(0, 8, $this->footer_text . '  --  Page ' . $this->pageNo().' of '.$this->getAliasNbPages(), 
                        0, false, 'C', 0, '', 0, false, 'T', 'B');
                }
            }
        }

        public function AddMySection($ciniki, $tnid, $section) {

            $page_width = $this->getPageWidth() - $this->left_margin - $this->right_margin;
            $page_height = $this->getPageHeight() - $this->top_margin - $this->header_height - $this->footer_height;

            //
            // Calculate how much height is required for this step
            //
            $title_height = 0;
            $newpage = 'no';
            if( $section['subtitle'] != '' ) {
                $this->SetFont('', 'B', '16');
                $title_height = $this->getStringHeight($page_width, $section['subtitle']);
            }
            $image = null;
            $img_box_height = 0;
            if( $section['image_id'] > 0 ) {
                $rc = ciniki_images_loadImage($ciniki, $tnid, $section['image_id'], 'original');
                if( $rc['stat'] == 'ok' ) {
                    $image = $rc['image'];
                    $img_height = (($this->left_col_width / $image->getImageWidth()) * $image->getImageHeight());
                    //
                    // Check if there is not room on this page
                    //
                    if( $img_height > ($this->getPageHeight() - $this->GetY() - $title_height - $this->footer_height - 10) ) {
                        $this->AddPage();
                        $newpage = 'yes';
                        $img_box_height = ($this->getPageHeight() - $this->GetY() - $title_height - $this->footer_height);
                    } else {
                        $img_box_height = $this->getPageHeight() - $title_height - $this->footer_height;
                    }
                }
            }
    
            $content_height = 0;
            if( $section['content'] != '' ) {
//                $section['content'] = preg_replace("/\n/", "<br/>", $section['content']);
                $this->SetFont('', '', '12');
                if( $image == null ) {
                    $content_height = $this->getStringHeight($this->left_col_width + $this->right_col_width, $section['content']);
                } else {
                    $content_height = $this->getStringHeight($this->right_col_width, $section['content']);
                }
                if( $content_height > ($this->getPageHeight() - $title_height - $this->footer_height) ) {
                    $lines = explode("\n", $section['content']);
                    $content = '';
                    $num_pages = 0;
                    foreach($lines as $line) {
                        $new_content = $content . $line . "\n";
                        // No image or second page of content
                        if( $image == null || $num_pages > 0 ) {
                            $content_height = $this->getStringHeight($this->left_col_width + $this->right_col_width, $new_content, true);
                        } else {
                            $content_height = $this->getStringHeight($this->right_col_width, $new_content, true);
                        }
                        error_log($content_height . '--' . $this->getPageHeight() . '::' . $this->GetY() . '::' . $title_height . '::' . $this->footer_height);
                        if( $content_height > ($this->getPageHeight() - $this->GetY() - $title_height - $this->footer_height) ) {
                            if( $num_pages == 0 ) {
                                $section['content'] = $content;
                            } else {
                                $section['content_' . $num_pages] = $content;
                            }
                            $num_pages++;
                            $content = '';
                        }
                        if( $content != '' || trim($line) != '' ) {
                            $content .= $line . "\n";
                        }
                    }
                    if( $content != '' ) {
                        if( $num_pages == 0 ) {
                            $section['content'] = $content;
                        } else {
                            $section['content_' . $num_pages] = $content;
                        }
                    }
                }
                elseif( $content_height > ($this->getPageHeight() - $this->GetY() - $title_height - $this->footer_height) ) {
                    $this->AddPage();
                    $newpage = 'yes';
                    $img_box_height = $this->getPageHeight() - $title_height - $this->footer_height;
                }
            }
            error_log($this->getPageHeight());
            error_log($content_height);
            error_log(print_r($section, true));

            //
            // Add the image title
            //
            if( $section['subtitle'] != '' ) {
                $this->SetFont('', 'B', '16');
                $this->Cell($page_width, 11, $section['subtitle'], 0, 1, 'L');
            }
        
            //
            // Add the content
            //
            $start_y = $this->GetY();
            if( $section['content'] != '' ) {
                $section['content'] = preg_replace("/\n/", "<br/>", $section['content']);
                $this->SetFont('', '', '12');
                if( $image == null ) {
                    $this->SetX($this->left_margin + $this->middle_margin);
                    $this->MultiCell($page_width, 8, $section['content'], 0, 'L', false, 1, '', '', true, 0, true, true, 0, 'T');
                } else {
                    $this->SetX($this->left_margin + $this->left_col_width + $this->middle_margin);
                    $this->MultiCell($this->right_col_width, 8, $section['content'], 0, 'L', false, 1, '', '', true, 0, true, true, 0, 'T');
                }
            }
            $end_y = $this->GetY();

            //
            // Add the image
            //
            if( $image != null ) {
                $this->setY($start_y+1);
                $this->SetLineWidth(0.25);
                $this->SetDrawColor(50);
                $img = $this->Image('@'.$image->getImageBlob(), '', '', $this->left_col_width, $img_box_height, 
                    'JPEG', '', '', true, 300, '', false, false, 
                    array('LTRB'=>array('color'=>array(128,128,128))), 'CT');
                $this->Ln();
                $this->Ln(5);
            }

            if( $end_y > $this->GetY() ) {
                $this->SetY($end_y);
            }

            //
            // Check if more content
            //
            if( isset($num_pages) ) {
                for($i = 1; $i <= $num_pages; $i++ ) {
                    if( $section['content_' . $i] != '' ) {
                        $this->AddPage();
                        if( $section['subtitle'] != '' ) {
                            $this->SetFont('', 'B', '16');
                            $this->Cell($page_width, 11, $section['subtitle'] . ' (continued...)', 0, 1, 'L');
                            $this->Ln(2);
                        }
                        $section['content_' . $i] = preg_replace("/\n/", "<br/>", $section['content_' . $i]);
                        $this->SetFont('', '', '12');
                        $this->SetX($this->left_margin + $this->middle_margin);
                        $this->MultiCell($page_width, 8, $section['content_' . $i], 0, 'L', false, 1, '', '', true, 0, true, true, 0, 'T');
                    }
                }
            }
        }
    }

    //
    // Start a new document
    //
    $pdf = new MYPDF('P', PDF_UNIT, 'LETTER', true, 'UTF-8', false);

    $pdf->title = $args['title'];

    // Set PDF basics
    $pdf->SetCreator('Ciniki');
    $pdf->SetAuthor($tenant_details['name']);
    $pdf->footer_text = $tenant_details['name'];
    $pdf->SetTitle($args['title']);
    $pdf->SetSubject('');
    $pdf->SetKeywords('');

    // set margins
    $pdf->header_height = 22;
    $pdf->footer_height = 10;
    $pdf->top_margin = 10;
    $pdf->left_margin = 20;
    $pdf->right_margin = 20;
    $pdf->middle_margin = 5;
    $pdf->left_col_width = 70;
    $pdf->right_col_width = $pdf->getPageWidth() - $pdf->left_margin - $pdf->left_col_width - $pdf->middle_margin - $pdf->right_margin;
    $pdf->SetMargins($pdf->left_margin, $pdf->header_height, $pdf->right_margin);
    $pdf->SetHeaderMargin($pdf->top_margin);
    $pdf->setPageOrientation('P', false);
    $pdf->SetFooterMargin(0);

    // Set font
    $pdf->SetFont('times', 'BI', 10);
    $pdf->SetCellPadding(0);

    if( isset($args['doublesided']) ) {
        $pdf->doublesided = $args['doublesided'];
    }

    //
    // Check if coverpage is to be outputed
    //
    if( isset($args['coverpage']) && $args['coverpage'] == 'yes' ) {
        $pdf->coverpage = 'yes';
        $pdf->title = '';
        if( isset($args['title']) && $args['title'] != '' ) {
            $title = $args['title'];
            $pdf->footer_text .= '  --  ' . $args['title'];
        } else {
            $title = "Tutorials";
        }
        $pdf->pagenumbers = 'no';
        $pdf->AddPage('P');
        
        if( isset($args['coverpage-image']) && $args['coverpage-image'] > 0 ) {
//          $img_box_width = 180;
            $img_box_width = $pdf->getPageWidth() - $pdf->left_margin - $pdf->right_margin;
            $img_box_height = 150;
            $rc = ciniki_images_loadCacheOriginal($ciniki, $tnid, $args['coverpage-image'], 2000, 2000);
            if( $rc['stat'] == 'ok' ) {
                $image = $rc['image'];
                $pdf->SetLineWidth(0.25);
                $pdf->SetDrawColor(50);
                $img = $pdf->Image('@'.$image, '', '', $img_box_width, $img_box_height, 'JPEG', '', '', true, 300, '', false, false, 0, 'CT');
            }
            $pdf->SetY(-50);
        } else {
            $pdf->SetY(-100);
        }


        $pdf->SetFont('times', 'B', '30');
        $pdf->MultiCell(180, 5, $title, 0, 'C', false, 1, '', '', true, 0, false, true, 0, 'T');
        $pdf->endPage();
        if( $pdf->doublesided == 'yes' ) {
            $pdf->AddPage();
            $pdf->Cell(0, 0, '');
            $pdf->endPage();
        }
    }
    $pdf->pagenumbers = 'yes';

    //
    // Add the tutorials items
    //
    $page_num = 1;
    $pdf->toc_categories = 'no';
    if( count($categories) > 1 ) {
        $pdf->toc_categories = 'yes';
    }
    if( isset($args['toc']) && $args['toc'] == 'yes' ) {
        $pdf->toc = 'yes';
    }

    if( $pdf->toc == 'yes' && $pdf->doublesided == 'yes' ) {
        $pdf->AddPage();
        $pdf->Cell(0, 0, '');
        $pdf->endPage();
    }

    foreach($categories as $cid => $category) {
        $tutorial_num = 1;
        foreach($category['tutorials'] as $tid => $tutorial) {
            if( isset($args['removetext']) && $args['removetext'] != '' ) {
                $tutorial['title'] = preg_replace('/' . $args['removetext'] . '/', '', $tutorial['title']);
            }
            $pdf->title = $tutorial['title'];
            // Start a new page
            $pdf->AddPage('P');
            $pdf->SetFillColor(255);
            $pdf->SetTextColor(0);
            $pdf->SetDrawColor(51);
            $pdf->SetLineWidth(0.15);
            // 
            // Add introduction to tutorial
            //
            $pdf->AddMySection($ciniki, $tutorial['tnid'], 
                array('image_id'=>0, 'subtitle'=>'', 'content'=>$tutorial['synopsis']));
            // Add a table of contents bookmarks
            if( isset($args['toc']) && $args['toc'] == 'yes' ) {
                if( $pdf->toc_categories == 'yes' && $tutorial_num == 1 ) {
                    $pdf->Bookmark($category['name'], 0, 0, '', '');
                }
                if( $pdf->toc_categories == 'yes' ) {
                    $pdf->Bookmark($pdf->title, 1, 0, '', '');
                } else {
                    $pdf->Bookmark($pdf->title, 0, 0, '', '');
                }
            }


            $step_num = 1;
            if( isset($tutorial['steps']) ) {
                //
                // Count total steps, then setup titles
                //
                $num_steps = 0;
                $step_num = 0;
                $substep_num = 0;
                foreach($tutorial['steps'] as $sid => $step) {
                    if( $step['content_type'] == 10 ) {
                        $num_steps++;
                    }
                }
                foreach($tutorial['steps'] as $sid => $step) {
                    if( $step['content_type'] == 10 ) {
                        $step_num++;
                        $substep_num = 0;
                        $full_title = 'Step ' . $step_num . ' of ' . $num_steps . ' - ' . $step['title'];
                    }
                    elseif( $step['content_type'] == 20 ) {
                        $substep_num++;
                        $full_title = 'Step ' . $step_num . chr(96+$substep_num) . ' of ' . $num_steps . ' - ' . $step['title'];
                    }
                    elseif( $step['content_type'] == 50 ) {
                        $substep_num++;
                        $full_title = $step['title'];
                    }
                    $step['number'] = $step_num;
                    $pdf->AddMySection($ciniki, $tutorial['tnid'], 
                        array('image_id'=>$step['image1_id'], 'subtitle'=>$full_title, 'content'=>$step['content']));
                    $page_num++;
                }
            }
            $tutorial_num++;
        }
    }

    if( isset($args['toc']) && $args['toc'] == 'yes' ) {
        $pdf->title = 'Table of Contents';
        $pdf->addTOCPage();
        $pdf->Ln(8);
        $pdf->SetFont('', '', 14);
        $pdf->pagenumbers = 'no';
        $pdf->addTOC((($pdf->coverpage=='yes')?($pdf->doublesided=='yes'?3:2):0), 'courier', '.', 'INDEX', 'B');
        $pdf->pagenumbers = 'yes';
        $pdf->endTOCPage();
    }

    return array('stat'=>'ok', 'pdf'=>$pdf);
}
?>
